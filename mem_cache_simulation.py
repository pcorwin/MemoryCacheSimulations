...
def debug():
    ...
    print(f"\tCache Block Size:\t{cache_block_size} Bytes")
    print(f"\t# of Cache Blocks:\t{int(num_cache_blocks)}")
    print(f"\t# of Cache Sets:\t{int(num_cache_sets)}")
    print(f"\t{associativity}-Way Associativity")
...
def read_word(address):
    ...
    open_index = cache.tag_queue[0]
    for i in range(associativity - 1, 0, -1):
        if (cache.tag_queue[i] != tag_number):
            h_or_m = "miss"
        else:
            h_or_m = "hit"
            cache.tag_queue[i], cache.tag_queue[associativity - 1] = cache.tag_queue[associativity - 1], \
            cache.tag_queue[i]
            break;
    if (h_or_m == "miss"):
